...
final class KeyboardShortcutSettingsFileStoreStartupTests: XCTestCase {
    ...
    func testSettingsFileStoreAppliesSurfaceCycleOrder() throws {
        let defaults = UserDefaults.standard
        let key = AppCatalogSection().surfaceCycleOrder.userDefaultsKey
        try preservingDefaults(keys: [key, settingsFileBackupsDefaultsKey, importedManagedDefaultsKey]) {
            defaults.removeObject(forKey: key)
            defaults.removeObject(forKey: settingsFileBackupsDefaultsKey)
            defaults.removeObject(forKey: importedManagedDefaultsKey)
            ...
            XCTAssertEqual(defaults.string(forKey: key), "mostRecentlyUsed")
        }
    }
